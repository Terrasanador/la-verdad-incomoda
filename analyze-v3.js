...
function addPolicy(req) {
  ...
  if (key) {
    req.body = { ...body, [key]: `${body[key]}${POLICY}${ATTRIBUTION_POLICY}${CHECKABLE_CLAIM_POLICY}${POLITICAL_CONTEXT_POLICY}${BOOK_EVIDENCE_POLICY}${MEDIA_FORENSICS_POLICY}${predictionPolicy}` };
  } else if (typeof body.url === 'string' && body.url.trim()) {
    req.body = { ...body, consulta: `${body.url}${POLICY}${ATTRIBUTION_POLICY}${CHECKABLE_CLAIM_POLICY}${POLITICAL_CONTEXT_POLICY}${BOOK_EVIDENCE_POLICY}${MEDIA_FORENSICS_POLICY}${predictionPolicy}` };
  }
}
...
function esAtribucionOPeriferica(item) {
  ...
  const atribucion = /\b(?:dijo|afirmo|declaro|publico|escribio|difundio|aseguro|sostuvo|acuso|denuncio|imputo|formulo|presento|reporto|compartio|emitio|reprodujo|pidio|exigio|advirtio|cuestiono|califico|llamo)\b/.test(afirmacion) ||
    /\b(?:video|audio|grabacion|clip|publicacion|post)\b.{0,90}\b(?:contiene|incluye|muestra|reproduce|se escucha|se oye)\b/.test(afirmacion) ||
