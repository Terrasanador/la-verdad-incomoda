...
const texto = tieneTexto
  ...
  : "Analiza el archivo adjunto.";
    ...
    const instrucciones = [
      ...
      "11. Antes de asignar un veredicto, identifica una proposición factual completa y comprobable. La identidad de una URL, su redirección, la cuenta autora o la coincidencia literal de una vista previa son datos técnicos de recuperación, no prueba de verdad del contenido.",
      "12. Si el texto recuperado es una frase fragmentaria, coloquial o dependiente de referentes ausentes —por ejemplo 'te lo dije', 'mira esto', 'y es domingo' o solo emojis— no inventes la tesis faltante: usa INFORMACIÓN INSUFICIENTE y NO VERIFICABLE, con credibilidad null.",
      "13. Las herramientas para limpiar, expandir o visualizar enlaces y la presencia del mismo usuario en otra plataforma no son evidencia favorable de una afirmación factual.",
      "14. Cuando no exista una afirmación factual identificable, omite auditorías de orientación, financiamiento, reputación, intención, patrón, coordinación y bots; explica brevemente qué contexto falta.",
...
"Una fuente anónima es una atribución que debe evaluarse, no corroboración independiente. En acusaciones sobre salud, consumo de sustancias, delitos, vida privada o conducta actual exige evidencia directa, pertinente e independientemente corroborada.",
"Si solo está confirmado que alguien dijo, publicó o repitió X y X permanece NO DEMOSTRADA, nunca cierres con VERDADERO, CIERTA ni PARCIALMENTE CIERTA. Usa INFORMACIÓN INSUFICIENTE/NO VERIFICABLE si X no puede resolverse y FALSO/FALSA solo cuando evidencia suficiente la contradiga materialmente.",
"Si la fuente original usa palabras como supuesto, rumor, testimonio anónimo o no comprobado, conserva esa incertidumbre y no la conviertas en un hecho confirmado.",
"Aplica este mismo estándar a Anabel Hernández, Atypical TV, Carlos Salinas Pliego, Chumel Torres, Luisito Comunica, Adela Micha, Latinus, cuentas oficialistas, autoridades y cualquier otra fuente. El historial documentado determina el rigor del contraste, no el veredicto por sí solo.",
...
"Una ficha editorial, contraportada, página de venta, Google Books, reseña o entrevista promocional confirma la existencia de una obra y describe su tesis; no corrobora las acusaciones del libro. Exige la evidencia subyacente y contraste independiente.",
"No uses un libro anterior para validar automáticamente otro libro, video o acusación posterior. Comprueba título, edición, fecha, pasaje y documentos específicos.",
"Si no recuperaste la transcripción completa del video ni examinaste el pasaje íntegro del libro, no afirmes que la obra demuestra su tesis ni atribuyas a ella documentos o testimonios que no revisaste.",
"Casos particulares de corrupción o infiltración no demuestran expresiones totalizantes como que el crimen organizado es todo el sistema o que todas las instituciones ocultan una realidad. Verifica el alcance exacto.",
...
${texto}${bloqueExtraccion}`
    ...
    try {
      ...
      if (openAIResponse.status === 429) {
        ...
        const quotaBlocked = /(?:insufficient_quota|billing|quota_exceeded|credit_balance_exhausted)/i.test(firstCode);
    ...
    if (!openAIResponse.ok) {
      ...
      if (openAIResponse.status === 429) {
        ...
        const quotaBlocked = /(?:insufficient_quota|billing|quota_exceeded|credit_balance_exhausted)/i.test(providerCode);
        ...
        return res.status(429).json({
          ...
          retry_after_seconds: quotaBlocked
            ? null
