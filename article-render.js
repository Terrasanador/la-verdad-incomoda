...
<main><nav aria-label="Ruta de navegación" class="meta"><a href="/">Inicio</a> · <a href="/articles.html">Artículos</a> · ${esc(article.category)}</nav><div class="tag">${esc(article.category)}</div><h1>${esc(article.title)}</h1><p class="summary">${esc(article.summary || "")}</p><p class="meta"><a href="/autores.html">${esc(article.author)}</a> · Publicado: ${published}${modified !== published ? ` · Actualizado: ${modified}` : ""} · ${(article.sources || []).length} fuentes</p>${article.verdict ? `<section class="verdict-box"><small>VEREDICTO</small><strong>${esc(article.verdict)}</strong>${article.claim ? `<p><b>Afirmación revisada:</b> ${esc(article.claim)}</p>` : ""}</section>` : ""}<div class="content">${paragraphs}</div>${sources ? `<section class="sources"><h2>Fuentes consultadas</h2><p>Enlaces directos utilizados para que el lector pueda reproducir la revisión:</p><ul>${sources}</ul></section>` : ""}${related.length ? `<section class="sources"><h2>Artículos relacionados</h2><ul>${related.map((item) => `<li><a href="/articulos/${encodeURIComponent(item.slug)}">${esc(item.title)}</a></li>`).join("")}</ul></section>` : ""}<section class="sources"><h2>Transparencia y responsabilidad</h2><p>Este artículo fue elaborado mediante consulta de fuentes públicas. La inteligencia artificial puede ayudar a localizar y organizar materiales, pero no se considera una fuente ni sustituye la revisión editorial. La conclusión se basa en los documentos enlazados y puede corregirse si aparece evidencia mejor. Consulta nuestra <a href="/metodologia.html">metodología</a>, <a href="/politica-editorial.html">política editorial</a>, <a href="/autores.html">responsabilidad de autoría</a> y <a href="/correcciones.html">procedimiento de correcciones</a>.</p></section></main><footer style="width:min(820px,calc(100% - 24px));margin:40px auto 20px;padding:20px 0;border-top:1px solid #373740;color:#b8b8c1">© 2026 La Verdad Incómoda · <a href="/privacidad.html">Privacidad</a> · <a href="/terminos.html">Términos</a> · <a href="/contacto.html">Contacto</a> · <a href="/quienes-somos.html">Quiénes somos</a></footer></body></html>`;
}
